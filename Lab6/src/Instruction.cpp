...
/*
构造函数接受五个参数：
unsigned opcode：表示指令的操作码，用于标识执行何种二元操作（如加法、减法等）。
Operand *dst：目的操作数，表示指令的结果将存储在这个操作数中。
Operand *src1 和 Operand *src2：表示参与操作的两个源操作数。
BasicBlock *insert_bb：表示指令将被插入的基本块。
将 opcode、dst、src1 和 src2 添加到 operands 数组中。
通过调用 setDef 方法，将目的操作数 dst 标记为当前指令的定义（def）。
通过调用 addUse 方法，将源操作数 src1 和 src2 标记为当前指令的使用（use）。
*/
...
/*
常量（ConstantSymbolEntry）：
如果操作数是常量，函数会创建一个类型为 IMM（立即数）的 MachineOperand 对象，
并使用常量的值初始化它。

临时变量（TemporarySymbolEntry）：
如果操作数是临时变量，函数会创建一个类型为 VREG（虚拟寄存器）的 
MachineOperand 对象，并使用临时变量的标签（label）初始化它。

变量（IdentifierSymbolEntry）：
如果操作数是变量，函数首先检查该变量是否为全局变量。如果是全局变量，
函数会创建一个类型为 IMM（立即数）的 MachineOperand 对象，并使用变量的名称
初始化它。在这个简化的代码中，对于非全局变量的情况，程序会调用 exit(0) 来退出
程序，暗示需要根据实际情况进行扩展或修改。
*/
...
void AllocaInstruction::genMachineCode(AsmBuilder *builder)
{
    ...
    int size = se->getType()->getSize() / 8; //获取当前函数和分配空间大小
    fprintf(stderr, "1234567891012345678%s: %d: %d\n", se->toStr().c_str(), size, se->getType()->getSize());
    if (size < 0)
        size = 4;
    int offset = cur_func->AllocSpace(size);//分配栈空间
...
/*
检查操作数是否为全局变量：检查操作数是否为全局变量，如果是，
则生成加载全局变量的机器指令。首先创建目的操作数 dst，然后生成两个临时寄存器
 internal_reg1 和 internal_reg2。接着生成源操作数 src，并生成两个
  LoadMInstruction 指令，一个用于加载地址，另一个用于从地址处加载值。

检查操作数是否为局部临时变量：检查操作数是否为局部临时变量，
并且是否有对应的分配指令。如果满足条件，则生成加载局部临时变量的机器指令。
首先创建目的操作数 dst，然后生成两个源操作数，一个用于加载栈底指针 src1，
另一个用于加载偏移量 src2。接着生成 LoadMInstruction 指令，用于加载指定
偏移量处的值。

处理其他情况：如果不满足以上两种情况，表示操作数既不是全局变量也不是
局部临时变量，则生成普通的加载机器指令。首先创建目的操作数 dst 和源操作数 
src，然后生成 LoadMInstruction 指令，用于从地址处加载值。
*/
