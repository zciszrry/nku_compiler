...
void BinaryExpr::genCode()
{
    ...
    else if (op >= LESS && op <= NOTEQUAL) // LESS, MORE, LEQ, MEQ, EQ, NE
    {
...
{
    // Todo
    cond->typeCheck();  // 对条件表达式进行类型检查
    if(thenStmt)        
        thenStmt->typeCheck();

    IntType *t = dynamic_cast<IntType*>(cond->getSymPtr()->getType());

    // 检查条件表达式是否为布尔类型
    if (!t->isBool())
    {
        // 如果条件表达式不是布尔类型，将其转换为布尔类型
...
{
    // 遍历语法树，找到最近的 WhileStmt
    Node *currentNode = this->getNext();
    while (currentNode != nullptr)
    {
        WhileStmt *whileNode = dynamic_cast<WhileStmt *>(currentNode);
        if (whileNode != nullptr)
        {
            // Break 出现在 While 语句中，检查通过
            return;
        }
        currentNode = currentNode->getNext();
    }

    // 如果没有找到 WhileStmt，说明 Break 语句不在循环中，报错
    fprintf(stderr, "Error: Break statement must be inside a while loop.\n");
    assert(false);  // 或者执行其他错误处理逻辑
}
...
void ContinueStmt::typeCheck()
{
    ...
    while (currentNode != nullptr)
    {
        ...
        if (whileNode != nullptr)
        {
            // Continue 出现在 While 语句中，检查通过
            return;
        }
        currentNode = currentNode->getNext();
    }

    // 如果没有找到 WhileStmt，说明 Continue 语句不在循环中，报错
    fprintf(stderr, "Error: Continue statement must be inside a while loop.\n");
    assert(false);  // 或者执行其他错误处理逻辑
}
...
{
    id->typeCheck();        //确保标识符的类型是有效
    if (expr)               //检查声明语句中可能存在的赋值表达式 expr
        expr->typeCheck();
}
// 返回语句
...
void FunctionDef::typeCheck()
{
    ...
    if (!stmt->checkRet(retType))       //判断return 语句操作数和函数声明的返回值类型是否匹配
    {
...
获取函数的返回值类型（retType）。
调用 stmt->checkRet(retType) 进行检查，确保函数体中的返回语句符合函数声明的返回值类型。
如果检查失败，输出错误信息并终止程序。

ReturnStmt::checkRet(Type *retType)：
如果没有返回值 (!retValue)，则检查函数声明的返回值类型是否为 void。如果不是，则输出错误信息。
如果有返回值，获取返回值的类型（valueType）。
检查返回值类型和函数声明的返回值类型是否匹配：
如果是整数或常量整数，且匹配，则返回 true。
如果返回值类型为函数类型，并且该函数类型的返回值是整数，则返回 true。
否则，输出错误信息。
*/
...
void BinaryExpr::typeCheck()
{
    ...
    // 判断是否为函数类型，如果是，则获取其返回类型
    if (type1->isFunc())
    {
        FunctionType *fun = static_cast<FunctionType *>(type1);
        type1 = fun->getRetType();
    }

    if (type2->isFunc())
    {
        FunctionType *fun = static_cast<FunctionType *>(type2);
    ...
    IntType *t1 = static_cast<IntType *>(type1);
    IntType *t2 = static_cast<IntType *>(type2);
...
{
    expr->typeCheck();  // 对子表达式进行类型检查

    IntType *type = dynamic_cast<IntType *>(expr->getSymPtr()->getType());
